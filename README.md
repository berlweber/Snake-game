...
-   adjustments:
    ...
    - make a seperate message for starting/finishing the game and game instructions, and place them in different places.
    - make the snakes head visible that its a head.
    - add a theme or background for the whole page.
-   make it responsive:
    - To adjust to different screen sizes.
    - To adjust to window resizing.
-   Add features:
    - Speed-up when holding down a navigation key, needed especially at the beggining of the game.
    - Show current score thoughout the game.
    - Show current speed in a px/scnd format.
    - keep a high-score record.
    - enable navigation also through a set of home-row letter-keys, to make it more easy to nevigate over a longer period of time.
    - enable start and pause through clicking on the whole game field.
    - add sounds for each move of the snake and when eating.
